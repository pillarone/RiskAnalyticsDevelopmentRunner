 ...
 */
class ScalarCachePimp[K, V](private val impl: GridCache[K, V]) extends ScalarCacheProjectionPimp[K, V](impl)
    with Ordered[GridCache[K, V]] {
    if (impl == null)
     ...
     */
    def compare(that: GridCache[K, V]): Int = that.name.compareTo(impl.name)
}